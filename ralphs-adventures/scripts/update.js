...
function updateEnemy(enemyList, enemyCanMove) {
  enemyList.map((enemy) => {
    ...
    if (enemyCanMove) {
      ...
      if((movePlain[enemy.y.keepInRange(0,8)+1][enemy.x] === player.symbol ||
         movePlain[enemy.y.keepInRange(1,9)-1][enemy.x] === player.symbol ||
         movePlain[enemy.y][enemy.x.keepInRange(0,8)+1] === player.symbol ||
         movePlain[enemy.y][enemy.x.keepInRange(1,9)-1] === player.symbol
        )&& playerImmune) {
        player.health--;
...
let gameWindow = document.getElementById("game-window"); //get html game window
...
setInterval(() => {
  ...
  //loop once player is dead
    ...
    if(!removedOneItem)
      removedOneItem = popDraw(movePlain, spot, bennyList); //shots second

    //reset game if r is pressed
    if (keyDown.get(controls.reset))
       setupGame();
  }
